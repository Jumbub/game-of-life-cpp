...
void setBenchmarkBoard(Board& board) {
  ...
  for (uint ry = 1; ry < height + 1; ++ry) {
    for (uint rx = 1; rx < width + 1; ++rx) {
      ...
      if (y > height / 2) {
        if (x < width / 2)
          board.output[INDEX(i)] = rand() % 2 ? ALIVE : DEAD;
        if (x >= width / 2)
          board.output[INDEX(i)] = (x / 8) % 2 != (y / 8) % 2 ? ALIVE : DEAD;
      } else {
        ...
        if (breederY > 0 && breederY < BREEDER_HEIGHT && x < BREEDER_WIDTH) {
          board.output[INDEX(i)] = BREEDER[breederY][x] ? ALIVE : DEAD;
        } else {
          board.output[INDEX(i)] = DEAD;
        }
      }
    }
  }
